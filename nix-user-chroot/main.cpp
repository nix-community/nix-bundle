...
static int child_proc(const char *rootdir, const char *nixdir,
                      const char *working_dir, uint8_t clear_env,
...
static void usage(const char *pname) {
  ...
  fprintf(stderr, "\t-w <working_dir>\tset the working directory before "
...
#undef x
  ...
  int opt;
  while ((opt = getopt(argc, argv, "cen:m:dp:w:")) != -1) {
    switch (opt) {
    case 'c':
      clear_env = 1;
      break;
    ...
    case 'p':
      ...
      break;
    case 'w':
      working_dir = realpath(optarg, NULL);
      break;
    }
  }
  ...
  auto is_not_ancestor = [working_dir](DirMapping dir) {
    auto wd_path = filesystem::path(working_dir);
    auto base_path = filesystem::path(dir.src);
    auto mismatch_pair = mismatch(wd_path.begin(), wd_path.end(),
                                  base_path.begin(), base_path.end());
    return mismatch_pair.second != base_path.end();
  };
  bool not_mapped =
      all_of(dirMappings.begin(), dirMappings.end(), is_not_ancestor);
  auto wd_path = filesystem::path(working_dir);
  if (not_mapped && wd_path.string() != "/") {
    filesystem::path first_parent = *++wd_path.begin();
    dirMappings.push_back({"/" + first_parent.string(), first_parent.string()});
  }
  ...
  if (child < 0) {
    ...
    close(unrace[0]);
    return child_proc(rootdir, nixdir, working_dir, clear_env, dirMappings,
                      envMappings, argv[optind], argv + optind);
  } else {
    close(unrace[0]);
